...
Keep changes focused and describe the user-visible behavior they improve. Run `bash scripts/test.sh` and `bash scripts/build.sh` before opening a pull request. For installer, signing, or packaging changes, also run `bash Tests/install.sh`, `bash Tests/signing.sh`, and `bash scripts/package.sh`. Quit the destination app before building, or use a separate `HAND_MOUSE_BUILD_DIR`. Source builds reuse private signing state outside the repository; never include that state in a commit or artifact.
