...
Click defaults to on when no choice is saved. Only a manual checkbox change saves a new click preference. Practice turns clicks off for the current session, including after finishing, without overwriting the saved choice. Relaunching restores that choice or the default on; the camera still starts paused. Existing saved off values remain off, including older values that cannot be distinguished from an intentional choice.
...
Bring index + middle together to stop the pointer immediately before bending. The lock starts on the first confident observation at a fingertip gap of at most 30% of palm width, stays through holds and tap timeouts, and releases at 42% to avoid jitter. Separation reanchors movement at the held cursor so hand travel during the lock cannot cause a jump. Missing proximity measurements retain an existing lock; tracking interruption clears it. This stop gesture requires Click to be enabled and does not itself click.
...
The persistent guide highlights **Aim → Bend → Lift**. Practice and system control use the same next-action model: `Bend both fingers` during an incomplete bend, `Lift to click` only when the detector can accept release, and a specific retry hint after a timeout, incomplete bend, or missing finger landmarks. A canceled tap never retains the lift cue, even if the separate-fingers pointer lock remains engaged. Practice keeps Settings collapsed and shows target-hit feedback before the idle lock hint. The click gesture remains bend, then lift.
...
Steady aim is enabled by default and can be turned off in Settings. It reduces pointer travel during slow, careful hand movement while keeping normal travel for faster movements. It responds to hand movement, not button bounds or target recognition. The existing smoothing briefly settles at the fine adjustment; holding still does not then pull the pointer back toward the full-speed mapped position.
